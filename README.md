...
Ibox is built using clang and LLVM but also can be built with a cross-compiled Gcc as well if you want, You just need to edit the rules.mk file and uncomment **TARGET_ARCH**. Then change **CC** from 'clang' to 'cross-compiled gcc' as well as the **LDFLAGS**. Eventually I want to switch to a build script to detect and auto build based on your installed tools.  
